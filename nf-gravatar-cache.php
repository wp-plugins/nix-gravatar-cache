...
class NFGC_Gravatar_Cache {
	...
	public function get_cached_avatar( $source, $id_or_email, $size, $default, $alt, $args = array() ) {
		...
		$g_url_default_source = isset( $d_tmp[1] ) ? $d_tmp[1] : false;
		...
		$file_name      = md5( $gravatar_id.$g_url_default_source );
		$g_url_default_source_url = false == $g_url_default_source ? 404 : $g_url_default_source;
		...
		if ( isset( $nf_avatars_cache[ $gravatar_id ][ $size ] ) ) {
			$g_url  = $nf_avatars_cache[ $gravatar_id ][ $size ]['url'];
			$g_path = $nf_avatars_cache[ $gravatar_id ][ $size ]['path'];
		}

		if ( ! is_file( $g_path ) || (time() -filemtime( $g_path )) > $time ) {

			$curl_url = 'http://www.gravatar.com/avatar/'.$gravatar_id.'?s='.$size.'&r=G&d='.$g_url_default_source;
			...
			if ( 404 == $http_code ) {
				...
				if ( ! is_file( $g_path ) || (time() -filemtime( $g_path )) > $time ) {
					copy( 'http://www.gravatar.com/avatar/?d=mm', $g_path );
				}
			} else {
				// Check mime type
			...
			$nf_avatars_cache[ $gravatar_id ][ $size ]['url']  = $g_url;
			$nf_avatars_cache[ $gravatar_id ][ $size ]['path'] = $g_path;
